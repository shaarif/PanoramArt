...
<label id="query" style="visibility:hidden;">${Query}</label>
<label id="json" style="visibility:hidden;">${Results}</label>
   ...
   <div class = "footer">
        ...
        geocoder.geocode( { 'address': address}, function(results, status) { 
            if (status == google.maps.GeocoderStatus.OK) {
                ...
                google.maps.event.addListener(marker, 'mouseover', function() {
                    ...
                    //infowindow.setContent("<a onclick='$.fancybox.open({href:'http://localhost:8080/DemoEE/showReport?mname="+mname+"'}); return false;"+"'>"+mname+"</a>");
    ...
    { 
    	...
    	var url = "http://localhost:8080/DemoEE/showitems?mname="+mname+"&query="+query;
    	xhr.open("GET",url,true); 
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4) {
                var data = xhr.responseText;
                showcontent(mname,data);
            }
        };
        xhr.send(null);  
    }
    function showcontent(mname,data)
    {
    	document.getElementById('heading').innerHTML="<b>Search results for <i>"+query+"</i> at '"+mname+"'</b>";
    	var obj=eval ("(" + data + ")");
    	var myTable="<table>";
    	for (var i=0;i<obj.Artworks.length;i++) {
    		...
    		myTable+="<p><b>Artist:</b> <a href='http://localhost:8080/DemoEE/showArtist?aname="+obj.Artworks[i].Artist+"'>"+obj.Artworks[i].Artist+"</a></p>";
    ...
    function myFunction()
    {
    	document.getElementById('heading').innerHTML="<b>Search results for <i>"+query+"</i></b>";
